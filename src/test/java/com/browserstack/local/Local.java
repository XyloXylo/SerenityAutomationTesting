 ...
 */
/**
 * Created by aadm146 on 5/05/2017.
 */
public class Local {
    ...
    public void start(Map<String, String> options) throws Exception {
        ...
        if (options.get("binarypath") != null) {
            ...
            System.out.println("Setting up the local binary path");
            LocalBinary lb = new LocalBinary();
            binaryPath = lb.getBinaryPath();
            System.out.println("Here is the local binary path:"+binaryPath);
        }

        System.out.println("makeCommand start");
        makeCommand(options, "start");
        System.out.println("makeCommand finish");
        ...
        if (proc == null) {
            ...
            if(!obj.getString("state").equals("connected")){
                System.out.println("state is not connected");
                throw new LocalException(obj.getJSONObject("message").getString("message"));
            }
            else {
                System.out.println("Get the PID");
                pid = obj.getInt("pid");
            }
        }
    }
